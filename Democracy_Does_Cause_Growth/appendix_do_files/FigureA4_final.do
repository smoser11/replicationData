...
sum _b_shortrun_t
local mean=r(mean)
histogram _b_shortrun_t, bin(50) fraction fcolor(none) lcolor(black) kdensity kdenopts(lcolor(midblue) lwidth(thin)) ///
...
graph export "$project/appendix/mcarlo1_stat.eps", replace
...
graphregion(fcolor(white) lcolor(white) ifcolor(white) ilcolor(white)) plotregion(lcolor(black))
graph export "$project/appendix/mcarlo2_stat.eps", replace

sum _b_longrun
local mean=r(mean)
histogram _b_longrun, bin(50) fraction fcolor(none) lcolor(black) kdensity kdenopts(lcolor(midblue) lwidth(thin)) ///
ytitle(Fraction) xtitle(Estimate for permanent democratization) xline(`mean', lwidth(thin) lcolor(red)) xline(`true_longest', lwidth(thin) lcolor(red) lpattern(dash)) xlabel(0(5)22) ///
graphregion(fcolor(white) lcolor(white) ifcolor(white) ilcolor(white)) plotregion(lcolor(black))
graph export "$project/appendix/mcarlo3_stat.eps", replace

sum _b_persistence
local mean=r(mean)
histogram _b_persistence, bin(50) fraction fcolor(none) lcolor(black) kdensity kdenopts(lcolor(midblue) lwidth(thin)) ///
ytitle(Fraction) xtitle(Estimate for persistence of GDP) xline(`mean', lwidth(thin) lcolor(red)) xline(`true_persist', lwidth(thin) lcolor(red) lpattern(dash)) xlabel(0.9(0.005)0.97) ///
graphregion(fcolor(white) lcolor(white) ifcolor(white) ilcolor(white)) plotregion(lcolor(black))
graph export "$project/appendix/mcarlo4_stat.eps", replace
