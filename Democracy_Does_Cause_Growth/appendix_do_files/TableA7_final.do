...
foreach var in GDPpercapitaconstantLCUN rgdpna_full rgdpl2{

replace y=100*ln(`var')
...
*IV
quietly: xtivreg2 y l(1/4).y (dem=l(1/4).demreg) yy*, fe r cluster(wbcode2)
estimates store e4
nlcom (shortrun: _b[dem])  (lag1: _b[L.y])  (lag2: _b[L2.y])  (lag3: _b[L3.y])  (lag4: _b[L4.y]), post
vareffects
...
#delimit ;
estout e1 e2 e3 e4 e5   using "$project/appendix/TableGDPappendix_`var'.tex", style(tex) 
varlabels(dem "Democracy") 
cells(b(star fmt(%9.3f)) se(par)) 
stats(ar2p N N_g, fmt(%7.2f %7.0f  %7.0f) labels( "\input{appendix/TableGDPappendix_`var'_Add}  AR2 test p-value" "Observations" "Countries in sample" ))  
keep(dem) 
order(dem)
stardrop(dem)  
...
#delimit ;
estout e1_add e2_add e3_add e4_add e5_add   using "$project/appendix/TableGDPappendix_`var'_Add.tex", style(tex) 
